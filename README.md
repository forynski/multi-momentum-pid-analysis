...
This repository contains a **fully featured Jupyter Notebook** implementing advanced particle identification (PID) analysis across multiple momentum ranges, using machine learning techniques optimised with Optuna and featuring model caching for efficient repeated analysis.
...
- **Automatic hyperparameter optimisation via Optuna** for robust model tuning  
- **Model caching** to save and reload trained models and optimisation studies, dramatically reducing rerun times  
- Extensive **visualisation suite** including BDT output distributions, ROC curves, confusion matrices, and feature importances  
...
The data are split into four momentum ranges:
...
- Optimisation objective is ROC-AUC using a one-vs-one multi-class strategy  
...
- Trained models for each momentum range are serialised and saved to Google Drive  
- On subsequent notebook executions, saved models and Optuna studies are loaded automatically to skip expensive training and optimisation steps  
- Flags allow forcing re-training or re-optimisation if desired  
...
3. Run cells sequentially to:
   - Load data  
   - Optimise hyperparameters (or load pre-optimised)  
